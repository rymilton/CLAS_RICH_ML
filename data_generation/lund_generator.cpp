...
Particle generateParticle(int pdg, double mass, int sector = 1) {
  ...
  double p;
  // For protons, limit momentum to be above 3 GeV
  if (pdg == 2212 || pdg == -2212) {
    p = gRandom->Uniform(3, 8); // momentum in GeV
  } else {
    p = gRandom->Uniform(1.5, 8); // momentum in GeV
  }

  double theta = gRandom->Uniform(0, 40) * deg2rad;
